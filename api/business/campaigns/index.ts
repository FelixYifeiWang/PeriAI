...
async function normalizeCampaignInput(data: {
  ...
  status?: "processing" | "waiting_approval" | "negotiating" | "waiting_response" | "deal";
}) {
  try {
      ...
      status: parsed.status === 'deal'
        ...
        : parsed.status === 'waiting_approval'
          ...
          : parsed.status === 'negotiating'
            ? 'negotiating'
            : parsed.status === 'waiting_response'
              ? 'waiting_response'
              : 'processing',
    };
  } catch (error) {
